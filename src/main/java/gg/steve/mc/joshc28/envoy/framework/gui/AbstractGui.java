package gg.steve.mc.joshc28.envoy.framework.gui;
...
public abstract class AbstractGui {
     ...
     */
    public AbstractGui(ConfigurationSection section, String type, Integer size, int page) {
        this.inventoryID = UUID.randomUUID();
        if (!type.equalsIgnoreCase("none")) {
            this.inventory = Bukkit.createInventory(null, InventoryType.valueOf(type.toUpperCase()), ColorUtil.colorize(section.getString("name").replace("{page}", String.valueOf(page))));
        } else {
            this.inventory = Bukkit.createInventory(null, size, ColorUtil.colorize(section.getString("name").replace("{page}", String.valueOf(page))));
        }
