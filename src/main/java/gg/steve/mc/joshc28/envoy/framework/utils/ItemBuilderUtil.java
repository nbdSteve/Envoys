...
//                    return new ItemBuilderUtil(new HeadDatabaseAPI().getItemHead(parts[1]));
    ...
    public void addNBT(ConfigurationSection section) {
    ...
    public void addItemNBT(ConfigurationSection section) {
        nbtItem = new NBTItem(item);
        nbtItem.setBoolean("envoys.item", true);
        if (section.getBoolean("unbreakable")) {
            nbtItem.setBoolean("Unbreakable", true);
        }
    }

    public void addToolNBT(ConfigurationSection section) {
        nbtItem = new NBTItem(item);
        nbtItem.setBoolean("envoys.tool", true);
        if (section.getBoolean("unbreakable")) {
            nbtItem.setBoolean("Unbreakable", true);
        }
    }
